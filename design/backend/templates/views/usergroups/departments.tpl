...
{capture name="mainbox"}
    ...
    <form action="{""|fn_url}" method="post" id="departments_form" name="departments_form" enctype="multipart/form-data">
                    ...
                    <tr class="cm-row-status-{$department.status|lower} cm-longtap-target">
                        ...
                        <td width="6%" class="mobile-hide">
                            {capture name="tools_list"}
                                <li>{btn type="list" text=__("edit") href="usergroups.update_department?banner_id=`$department.department_id`"}</li>
                            {if $allow_save}
                                <li>{btn type="list" class="cm-confirm" text=__("delete") href="usergroups.delete_departments?banner_id=`$department.department_id`" method="POST"}</li>
                            {/if}
                            {/capture}
        ...
        {capture name="buttons"}
            {capture name="tools_list"}
                {if $departments}
                    <li>{btn type="delete_selected" dispatch="dispatch[usergroups.delete_departments]" form="departments_form"}</li>
                    
                {/if}
            {/capture}
            {dropdown content=$smarty.capture.tools_list class="mobile-hide"}
            {include file="buttons/save.tpl" but_name="dispatch[usergroups.update_departments]" but_role="action" but_target_form="departments_form" but_meta="cm-submit"}
        ...
        {capture name="adv_buttons"}
            ...
            {include file="common/tools.tpl" tool_href="usergroups.add_department" prefix="top" hide_tools="true" title="Добавить отдел" icon="icon-plus"}
